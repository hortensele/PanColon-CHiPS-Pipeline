...
def report_forest_plot_lr(meta_field, frame_clusters, directory, file_name, p_th=0.05):
    ...
    for label in labels:
        ...
        purity    = np.round(frame_label['Subtype Purity(%)'].astype(float).values,1).tolist()
        ...
        # Filter rows with any non-finite values across all numeric arrays.
        finite_mask = [
            np.isfinite(m) and np.isfinite(lo) and np.isfinite(up) and np.isfinite(pv)
            for m, lo, up, pv in zip(measure, lower, upper, pvalues)
        ]
        if not any(finite_mask):
            print('\t\tForest Plot - Skipping (all rows non-finite):', file_path)
            continue
        labs     = [v for v, k in zip(labs,     finite_mask) if k]
        measure  = [v for v, k in zip(measure,  finite_mask) if k]
        lower    = [v for v, k in zip(lower,    finite_mask) if k]
        upper    = [v for v, k in zip(upper,    finite_mask) if k]
        pvalues  = [v for v, k in zip(pvalues,  finite_mask) if k]
        subtype  = [v for v, k in zip(subtype,  finite_mask) if k]
        purity   = [v for v, k in zip(purity,   finite_mask) if k]
        counts   = [v for v, k in zip(counts,   finite_mask) if k]
        mean_tp  = [v for v, k in zip(mean_tp,  finite_mask) if k]
        max_tp   = [v for v, k in zip(max_tp,   finite_mask) if k]
        perc_pat = [v for v, k in zip(perc_pat, finite_mask) if k]
        ...
        try:
            ...
            print('\t\tForest Plot - Issue with %s: %s' % (file_path, e))
