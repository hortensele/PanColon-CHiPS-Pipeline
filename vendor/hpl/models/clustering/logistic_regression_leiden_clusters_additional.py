...
from models.clustering.data_processing_additional import *
...
''' ############## Logistic Regression ############## '''
# Get AUC performance.
def get_aucs(model, data, label, data_df=None, matching_field=None, groupby=None, i=None, alpha_path=None):
	train, valid, test, additional = data
	train_data, train_labels = train
	...
	# AUCs
	train_auc = roc_auc_score(train_labels[:,label], train_pred)
	aucs = [train_auc]
	if valid is not None:
		valid_auc = roc_auc_score(valid_labels[:,label], valid_pred)
		aucs.append(valid_auc)
	test_auc = roc_auc_score(test_labels[:,label], test_pred)
	aucs.append(test_auc)

	if additional is not None:
		additional_auc = roc_auc_score(additional_labels[:,label], additional_pred)
		aucs.append(additional_auc)

		# Save predictions
		if data_df is not None and matching_field is not None:
			add_df = data_df[data_df['Set'] == 'Additional']
			sample_ids = add_df[matching_field].values

			pred_labels = (additional_pred > 0.5).astype(int).flatten()
			pred_df = pd.DataFrame({
				'patient_id': sample_ids,
				'Y': additional_labels[:,label],
				'Y_hat': pred_labels,
				'p_0': 1 - additional_pred.flatten(),
				'p_1': additional_pred.flatten()
			})
			save_path = os.path.join(
				alpha_path, f"{groupby.replace('.', 'p')}_fold{i}_label{label}_additional_predictions.csv"
			)
...
def classification_performance_stats(data, leiden_clusters, frame_clusters, features, groupby, alpha, i, alpha_path, data_df, matching_field):
	train, valid, test, additional = data
	train_data, train_labels = train
	...
	for label in labels:
		...
		cms[label] = get_confusion_matrix(model, data, label)
	...
	for alpha in alphas:
		...
		for resolution in resolutions:
			...
			for i, fold in enumerate(folds):
				...
				# Logistic regression
				try:
					frame_clusters, aucs, cms = classification_performance_stats(
                    	data, leiden_clusters, frame_clusters, features,
                    	groupby, alpha, i, alpha_path, data_df, matching_field
                    )
		...
		if not valid_df.empty: columns.append('Valid AUC')
