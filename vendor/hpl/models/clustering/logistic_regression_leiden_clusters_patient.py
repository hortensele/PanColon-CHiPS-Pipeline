...
from models.clustering.data_processing_samples import *
...
def get_aucs(model, data, label, data_df=None, matching_field=None, groupby=None, i=None, alpha_path=None):
	...
	if additional is not None:
		...
		sample_ids = additional_df.samples.values
		# Build dataframe
		...
		pred_labels = (test_pred > 0.5).astype(int).flatten()
		# Patient/sample IDs
		train_df, valid_df, test_df, additional_df = data_df
		...
		pred_df = pd.DataFrame({
			...
			'Y': test_labels[:,label],
			'Y_hat': pred_labels,
			'p_0': 1-test_pred.flatten(),
			'p_1': test_pred.flatten()
		})
    
		pred_out_path = os.path.join(
			alpha_path, f"{groupby.replace('.', 'p')}_fold{i}_label{label}_test_predictions.csv"
		)
		pred_df.to_csv(pred_out_path, index=False)
	return aucs
...
def run_logistic_regression(alphas, resolutions, meta_folder, meta_field, matching_field, folds_pickle, h5_complete_path, h5_additional_path, force_fold, additional_as_fold, diversity_key=None,
	...
	for resolution in resolutions:
		...
		for i, fold in enumerate(folds):
			...
			data_res_folds[resolution][i] = {'data':data, 'features':features, 'frame_clusters':frame_clusters, 'leiden_clusters':leiden_clusters, 'data_df': data_df}
