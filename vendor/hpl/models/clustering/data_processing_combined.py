...
# Read CSV files.
def read_csvs(adatas_path, adatas_path_additional, hpc_label, additional_hpc_label, matching_field, groupby, i, fold, h5_complete_path, h5_additional_path, additional_as_fold=False, force_fold=None):

	# Rename labels.  
	hpc_label += "_hpc"    
	additional_hpc_label += "_hpc"  
	...
	if additional_as_fold:
		...
		leiden_clusters = np.unique(train_df[groupby].values.astype(str))
		...
		additional_leiden_clusters = np.unique(train_df[groupby].values.astype(int))
...
# 		test_csv     = os.path.join(adatas_path, '%s_test.csv' % adata_name)
		...
		if h5_additional_path is not None:
			adata_name     = h5_additional_path.split('/hdf5_')[1].split('.h5')[0] + '_%s__fold%s' % (groupby.replace('.', 'p'), i)
			additional_train_csv = os.path.join(adatas_path_additional, '%s.csv' % adata_name)
			if not os.path.isfile(additional_train_csv):
				additional_train_csv    = os.path.join(adatas_path_additional, '%s.csv' % adata_name)
			additional_valid_csv    = os.path.join(adatas_path_additional, '%s_valid.csv' % adata_name)
# 			additional_test_csv     = os.path.join(adatas_path_additional, '%s_test.csv' % adata_name)

		# Gather all sets for clustering. 
		train_df    = pd.read_csv(train_csv)  
		train_df[groupby] = [hpc_label + str(s) for s in train_df[groupby].values]
# 		test_df     = pd.read_csv(test_csv)
# 		test_df[groupby] = [hpc_label + str(s) for s in test_df[groupby].values]
		additional_train_df    = pd.read_csv(additional_train_csv)
		additional_train_df[groupby] = [additional_hpc_label + str(s) for s in additional_train_df[groupby].values]
# 		additional_test_df     = pd.read_csv(additional_test_csv)
# 		additional_test_df[groupby] = [additional_hpc_label + str(s) for s in additional_test_df[groupby].values]
		complete_df = pd.concat([train_df,additional_train_df], axis = 0, ignore_index = True)
# 		train_df    = pd.concat([train_df,additional_train_df], axis = 0, ignore_index = True)
# 		test_df    = pd.concat([test_df,additional_test_df], axis = 0, ignore_index = True)      
        
# 		complete_pd = [train_df, test_df]
		if os.path.isfile(valid_csv):
			valid_df = pd.read_csv(valid_csv)
			valid_df[groupby] = [hpc_label + str(s) for s in valid_df[groupby].values]
			complete_df.append(valid_df)
		if os.path.isfile(additional_valid_csv):
			additional_valid_df = pd.read_csv(additional_valid_csv)
			additional_valid_df[groupby] = [additional_hpc_label + str(s) for s in additional_valid_df[groupby].values]
			complete_df.append(additional_valid_df)
# 		complete_df  = pd.concat(complete_pd, ignore_index=True)
		...
		# Get clusters from the reference clustering train set.
		leiden_clusters = np.unique(complete_df[groupby].values.astype(str))
		additional_df = None
...
def cluster_purities(frame, population_df, meta_field, groupby):
	...
	frame_clusters[groupby]          = frame_clusters[groupby].astype(str)
...
def sample_representation(frame_classification, matching_field, sample, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_):
	# Space feature vector representation.
	# samples_features = [0]*len(leiden_clusters)
	samples_features = [0]*len(leiden_clusters)
	# Get cluster counts for sample.
	...
	# Map to vector representation dimensions.
	for clust_id, count in zip(clusters_slide, clusters_counts):
		samples_features[list(leiden_clusters).index(clust_id)] = count
	samples_features = np.array(samples_features, dtype=np.float64)
	samples_features[pd.Series(leiden_clusters).str.contains(hpc_label)] = np.array(samples_features[pd.Series(leiden_clusters).str.contains(hpc_label)])/np.sum(samples_features[pd.Series(leiden_clusters).str.contains(hpc_label)])
	samples_features[pd.Series(leiden_clusters).str.contains(additional_hpc_label)] = np.array(samples_features[pd.Series(leiden_clusters).str.contains(additional_hpc_label)])/np.sum(samples_features[pd.Series(leiden_clusters).str.contains(additional_hpc_label)])
	...
	if type_ == 'alr':
		...
		samples_features = multiplicative_replacement(np.reshape(samples_features, (1,-1)))
		samples_features[pd.Series(leiden_clusters).str.contains(hpc_label)] = clr(np.reshape(samples_features[pd.Series(leiden_clusters).str.contains(hpc_label)], (1, -1)))
		samples_features[pd.Series(leiden_clusters).str.contains(additional_hpc_label)] = clr(np.reshape(samples_features[pd.Series(leiden_clusters).str.contains(additional_hpc_label)], (1, -1)))
	elif type_ == 'ilr':
		samples_features = multiplicative_replacement(np.reshape(samples_features, (1,-1)))
...
def build_cohort_representations(meta_folder, meta_field, matching_field, groupby, fold_number, folds_pickle, h5_complete_path, h5_additional_path, hpc_label, additional_hpc_label, type_composition, min_tiles,
	...
	data, data_df, features = prepare_data_classes(dataframes, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_composition, min_tiles, use_conn=use_conn,
...
def prepare_set_representation(frame, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_, min_tiles):
	lr_data  = list()
	lr_label = list()
	i = 0
	for sample in pd.unique(frame[matching_field]):
		...
		sample_rep   = sample_representation(frame, matching_field, sample, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_)
...
def prepare_set_classes(frame, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_, min_tiles, use_conn=True, own_corr=True, use_ratio=False, top_variance_feat=100, keep_features=None,
						return_tiles=False):
	# Get slide representations just with % per cluster in slide.
	slide_rep_df = prepare_set_representation(frame, matching_field=matching_field, meta_field=meta_field, groupby=groupby, leiden_clusters=leiden_clusters, hpc_label=hpc_label, additional_hpc_label=additional_hpc_label, type_=type_, min_tiles=min_tiles)
...
def prepare_data_classes(dataframes, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_composition, min_tiles, use_conn=True, own_corr=True, use_ratio=False, top_variance_feat=100, return_tiles=False):
	train_df, valid_df, test_df, additional_df = dataframes

	# Create representations per sample: cluster % of total sample.
	train, train_slides_df, keep_features, features  = prepare_set_classes(train_df, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_=type_composition, min_tiles=min_tiles,
	...
	if valid_df is not None:
		valid, valid_slides_df, _,             _         = prepare_set_classes(valid_df, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_=type_composition, min_tiles=min_tiles,
	...
	test,  test_slides_df,  _,             _         = prepare_set_classes(test_df,  matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_=type_composition, min_tiles=min_tiles,
																		   use_conn=use_conn, own_corr=own_corr, use_ratio=use_ratio, top_variance_feat=top_variance_feat, keep_features=keep_features, return_tiles=return_tiles)
	additional           = None
	additional_slides_df = None
	if additional_df is not None:
		additional, additional_slides_df, _, _ = prepare_set_classes(additional_df, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_=type_composition, min_tiles=min_tiles,
...
def prepare_set_representation_survival(frame_classification, matching_field, groupby, leiden_clusters, type_, event_ind_field, event_data_field, min_tiles):
	...
	for sample in pd.unique(frame_classification[matching_field]):
		...
		sample_rep        = sample_representation(frame_classification, matching_field, sample, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_)
		lr_data.append(sample_rep)
