...
#     tiles_df['samples']  = tiles_df['caseid']
...
def institution_precense(adata_train, groupby, threshold=0.005):
    ...
    tiles_df = adata_train.obs
#     tiles_df['samples']  = tiles_df['caseid']
    tss_hpc = tiles_df[['samples', 'tiles']].groupby('samples').count()
    ...
    for hpc in np.unique(tiles_df[groupby]):
        ...
        hpc_df    = hpc_df.groupby('samples').count()
        hpc_df    = hpc_df.reset_index()[['samples', 'tiles']]
        hpc_df    = hpc_df.merge(tss_hpc, on='samples', how='inner')
