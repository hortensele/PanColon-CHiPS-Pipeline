...
def read_csvs(adatas_path, matching_field, groupby, i, fold, h5_complete_path, h5_additional_path, additional_as_fold=False, force_fold=None):
	...
	if additional_as_fold:
		...
		complete_df  = complete_df[complete_df[matching_field].astype(str).isin(all_samples)]
		additional_df = None
...
def cluster_diversity(frame, frame_clusters, groupby, diversity_key):
	...
	for cluster_id in np.unique(frame_samples[x].values):
		if len(frame_clusters[frame_clusters[x]==cluster_id]) > 0:
...
# Ratios between clusters in the sample
def cluster_ratios_sample(frame_classification, matching_field, sample, groupby, leiden_clusters, type_):
	# Space feature vector representation.
	samples_features = [0]*len(leiden_clusters)
	# Get cluster counts for sample.
	clusters_sample, clusters_counts = np.unique(frame_classification[frame_classification[matching_field]==sample][groupby], return_counts=True)
	# Map to vector representation dimensions.
	for clust_id, count in zip(clusters_sample, clusters_counts):
...
# Cluster interactions in the sample.
def cluster_conn_samples(frame, leiden_clusters, groupby, type='sample', own_corr=True, min_tiles=100, type_='percent'):
	if type=='sample':
		return cluster_conn_samples_interactions_sample(frame, leiden_clusters, groupby, own_corr=own_corr, min_tiles=min_tiles, type_=type_)
	elif type=='cluster':
		return cluster_conn_samples_interactions_clusters(frame, leiden_clusters, groupby, own_corr=True, min_tiles=100, type_='percent')
	else:
		print('Cluster interaction normalization not contemplated. Options: \'sample\' or \'cluster\'')
		return

# cluster connectivity for samples, interactions normalized per cluster.
def cluster_conn_samples_interactions_clusters(frame, leiden_clusters, groupby, own_corr=True, min_tiles=100, type_='percent'):
	all_samples = np.unique(frame.samples)
	heatmap    = list()

	# Get connectivity between clusters per sample.
	for sample in all_samples:
		sample_frame = frame[frame.samples==sample]
		if sample_frame.shape[0] < min_tiles:
			continue

		# Cluster proportions.
		sample_cluster_prop = np.zeros(len(leiden_clusters), dtype=np.float64)
		cluster_ids, counts = np.unique(sample_frame[groupby], return_counts=True)
		for cluster_id, count in zip(cluster_ids, counts):
			sample_cluster_prop[cluster_id] = count
		sample_cluster_prop = sample_cluster_prop/np.sum(sample_cluster_prop)
		...
		for cluster_id in leiden_clusters:
			...
			if sample_frame[sample_frame[groupby]==cluster_id].shape[0]!=0:
				# Get connections for those cluster tiles.
				cluster_conn, counts_conn = np.unique(sample_frame[sample_frame[groupby]==cluster_id][['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']], return_counts=True)
			...
			all_conn *= sample_cluster_prop[cluster_id]

			# Cluster connections to other clusters.
			sample_rep.extend(all_conn.tolist())

		# Normalize for all interactions -- Sample interaction normalization.
		sample_rep = np.array(sample_rep).astype(np.float64)
		sample_rep = multiplicative_replacement(np.reshape(sample_rep, (1,-1)))
		if type_ == 'clr':
			sample_rep = clr(np.reshape(sample_rep, (1,-1)))

		# Append sample representation.
		sample_rep = [sample] + sample_rep.tolist()
		heatmap.append(sample_rep)

	# Return data and labels.
	fields = ['samples'] + features
	heatmap = pd.DataFrame(heatmap, columns=fields)
	return heatmap, fields

# cluster connectivity for samples, interactions normalized for sample.
def cluster_conn_samples_interactions_sample(frame, leiden_clusters, groupby, own_corr=True, min_tiles=100, type_='percent'):
	all_samples = np.unique(frame.samples)
	heatmap    = list()
	...
	for sample in all_samples:
		...
		for cluster_id in leiden_clusters:
			...
			# Cluster connections to other clusters.
			sample_rep.extend(all_conn)

		# Normalize for all interactions -- Sample interaction normalization.
		# sample_rep = np.array(sample_rep+1).astype(np.float64)    # Avoid zero for compositional data.
		sample_rep = np.array(sample_rep).astype(np.float64)
		sample_rep = sample_rep/np.sum(sample_rep)
		sample_rep = multiplicative_replacement(np.reshape(sample_rep, (1,-1)))
		if type_ == 'clr':
			sample_rep = clr(np.reshape(sample_rep, (1,-1)))
...
def sample_representation(frame_classification, matching_field, sample, groupby, leiden_clusters, type_):
	# Space feature vector representation.
	# samples_features = [0]*len(leiden_clusters)
	samples_features = dict.fromkeys(leiden_clusters, 0)
	# Get cluster counts for sample.
	clusters_sample, clusters_counts = np.unique(frame_classification[frame_classification[matching_field]==sample][groupby], return_counts=True)
	# Map to vector representation dimensions.
	for clust_id, count in zip(clusters_sample, clusters_counts):
	...
	samples_features = np.array([*samples_features.values()], dtype=np.float64)
	samples_features = np.array(samples_features)/np.sum(samples_features)
...
def build_cohort_representations(meta_folder, meta_field, matching_field, groupby, fold_number, folds_pickle, h5_complete_path, h5_additional_path, type_composition, min_tiles,
	...
	complete_df.insert(0, 'samples', complete_df['samples'].apply(lambda x: '-'.join(x.split('-')[:3])))
...
# Prepare set representation with label, sample name, and # tiles.
def prepare_set_representation(frame, matching_field, meta_field, groupby, leiden_clusters, type_, min_tiles):
	...
	for sample in pd.unique(frame[matching_field]):
		...
		label_sample_meta = frame[frame[matching_field]==sample][meta_field].values[0]
		label_sample  = frame[frame[matching_field]==sample].samples.values[0]
		lr_data.append(sample_rep)
		lr_label.append((label_sample, label_sample_meta, num_tiles))
	...
	slide_rep_df['samples']   = lr_label[:,0].astype(str)
	slide_rep_df['tiles']    = lr_label[:,2].astype(int)
	slide_rep_df = slide_rep_df.set_index('samples')
...
def prepare_set_classes(frame, matching_field, meta_field, groupby, leiden_clusters, type_, min_tiles, use_conn=True, own_corr=True, use_ratio=False, top_variance_feat=100, keep_features=None,
						return_tiles=False):
	# Get sample representations just with % per cluster in sample.
	sample_rep_df = prepare_set_representation(frame, matching_field=matching_field, meta_field=meta_field, groupby=groupby, leiden_clusters=leiden_clusters, type_=type_, min_tiles=min_tiles)
	sample_rep_df = sample_rep_df.reset_index()
	if not return_tiles:
		sample_rep_df = sample_rep_df.drop(columns=['tiles'])

	# Get cluster connectivity within the samples.
	if use_conn:
		sample_con_df, con_fields = cluster_conn_samples(frame, leiden_clusters=leiden_clusters, groupby=groupby, own_corr=own_corr, min_tiles=min_tiles, type_=type_)
	...
	if use_conn or use_ratio:
		...
		if keep_features is None:
			...
			_, keep_features_var = feature_selector(sample_con_df[con_fields[1:]].to_numpy(), fields=con_fields, top_percent=top_variance_feat)
			keep_features.extend(keep_features_var[1:])
			keep_features = [matching_field] + keep_features
			sample_rep            = pd.merge(sample_rep_df, sample_con_df[keep_features], on=matching_field)
		else:
			sample_rep        = pd.merge(sample_rep_df, sample_con_df[keep_features], on=matching_field)
	...
	# Just cluster percentage.
		...
		sample_rep     = sample_rep_df.copy(deep=True)
		features      = copy.deepcopy(leiden_clusters.tolist())
		keep_features = None

	# # Remove features with 0 variance
	# var_selector = VarianceThreshold(threshold=1e-8)
	# sample_rep_array = var_selector.fit_transform(sample_rep[features])
	# mask = var_selector.get_support()
	# features = np.array(features)[mask].tolist()
	# sample_rep = sample_rep[['samples', meta_field] + features]  # adjust columns

	# Binary labels.
	labels_uniq = np.unique(sample_rep[meta_field].values.astype(int).tolist())
	labels = OneHotEncoder().fit_transform(np.array(sample_rep[meta_field].values.astype(int).tolist()).reshape(-1,1)).toarray()

	data = [sample_rep[features].to_numpy(), labels]
	return data, sample_rep, keep_features, features
...
def prepare_data_classes(dataframes, matching_field, meta_field, groupby, leiden_clusters, type_composition, min_tiles, use_conn=True, own_corr=True, use_ratio=False, top_variance_feat=100, return_tiles=False):
	...
	# Create representations per sample: cluster % of total sample.
	train, train_samples_df, keep_features, features  = prepare_set_classes(train_df, matching_field, meta_field, groupby, leiden_clusters, type_=type_composition, min_tiles=min_tiles,
	...
	valid           = None
	valid_samples_df = None
	if valid_df is not None:
		valid, valid_samples_df, _,             _         = prepare_set_classes(valid_df, matching_field, meta_field, groupby, leiden_clusters, type_=type_composition, min_tiles=min_tiles,
	...
	test,  test_samples_df,  _,             _         = prepare_set_classes(test_df,  matching_field, meta_field, groupby, leiden_clusters, type_=type_composition, min_tiles=min_tiles,
																		   use_conn=use_conn, own_corr=own_corr, use_ratio=use_ratio, top_variance_feat=top_variance_feat, keep_features=keep_features, return_tiles=return_tiles)
	additional           = None
	additional_samples_df = None
	if additional_df is not None:
		additional, additional_samples_df, _, _ = prepare_set_classes(additional_df, matching_field, meta_field, groupby, leiden_clusters, type_=type_composition, min_tiles=min_tiles,
	...
	return [train, valid, test, additional], [train_samples_df, valid_samples_df, test_samples_df, additional_samples_df], features
...
# Prepare set data with sample representations.
def prepare_set_representation_survival(frame_classification, matching_field, groupby, leiden_clusters, type_, event_ind_field, event_data_field, min_tiles):
	...
	sample_rep_df[event_data_field] = lr_event_data_field
	sample_rep_df[event_ind_field]  = lr_event_ind_field
...
def prepare_set_survival(frame, matching_field, groupby, leiden_clusters, type_, event_ind_field, event_data_field, min_tiles,
	...
	# Get sample representations just with % per cluster in sample.
	sample_rep_df = prepare_set_representation_survival(frame, matching_field=matching_field, groupby=groupby, leiden_clusters=leiden_clusters, type_=type_,
	...
	if use_conn or use_ratio:
		...
		if keep_features is None:
			...
			_, keep_features_var = feature_selector(sample_con_df[con_fields[1:]].to_numpy(), fields=con_fields, top_percent=top_variance_feat)
			keep_features.extend(keep_features_var[1:])
			keep_features = ['samples'] + keep_features
			sample_rep            = pd.merge(sample_rep_df, sample_con_df[keep_features], on=matching_field)
		else:
			sample_rep        = pd.merge(sample_rep_df, sample_con_df[keep_features], on=matching_field)
		features = leiden_clusters.tolist() + keep_features[1:] # Include all cluster %.
	...
	samples_df = sample_rep[[matching_field, event_data_field, event_ind_field]+features]
	return samples_df, keep_features, features
...
def prepare_data_survival(dataframes, groupby, leiden_clusters, type_composition, max_months, matching_field, event_ind_field, event_data_field, min_tiles,
	...
	# Get sample representations and labels
	train_samples_df, keep_features, features = prepare_set_survival(train_df, matching_field, groupby, leiden_clusters, type_composition, event_ind_field, event_data_field, min_tiles=min_tiles,
																	use_conn=use_conn, own_corr=True, use_ratio=use_ratio, top_variance_feat=top_variance_feat, keep_features=None)
	test_samples_df,  _,             _        = prepare_set_survival(test_df,  matching_field, groupby, leiden_clusters, type_composition, event_ind_field, event_data_field, min_tiles=min_tiles,
																	use_conn=use_conn, own_corr=True, use_ratio=use_ratio, top_variance_feat=top_variance_feat, keep_features=keep_features)
	valid_samples_df = None
	if valid_df is not None:
		valid_samples_df, _, _ = prepare_set_survival(valid_df, matching_field, groupby, leiden_clusters, type_composition, event_ind_field, event_data_field, min_tiles=min_tiles,
													 use_conn=use_conn, own_corr=True, use_ratio=use_ratio, top_variance_feat=top_variance_feat, keep_features=keep_features)
	additional_samples_df = None
	if additional_df is not None:
		additional_samples_df, _, _  = prepare_set_survival(additional_df, matching_field, groupby, leiden_clusters, type_composition, event_ind_field, event_data_field, min_tiles=min_tiles,
														   use_conn=use_conn, own_corr=True, use_ratio=use_ratio, top_variance_feat=top_variance_feat, keep_features=keep_features)
	...
	processed_data = [(train_samples_df, 'train'), (valid_samples_df, 'valid'), (test_samples_df, 'test'), (additional_samples_df, 'additional')]
