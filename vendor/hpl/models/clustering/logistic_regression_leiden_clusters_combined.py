...
from models.clustering.data_processing_combined import *
...
def summarize_run(alphas, resolutions, meta_folder, meta_field, min_tiles, folds_pickle, h5_complete_path, ylim=[0.0,1.01]):
...
def alpha_box_plot_auc_results(alphas, meta_folder, meta_field, min_tiles, h5_complete_path, labels, fontsize=18, ylim=[0.0,1.01]):
...
def run_circular_plots(resolutions, folder_meta_field, meta_field, matching_field, folds_pickle, h5_complete_path, h5_additional_path, hpc_label, additional_hpc_label, diversity_key=None):
	# Get folds from existing split.
	folds = load_existing_split(folds_pickle)
	...
	additional_cluster_path = h5_additional_path.split('hdf5_')[0]
	additional_cluster_path = os.path.join(additional_cluster_path, folder_meta_field)
	adatas_path_additional = os.path.join(additional_cluster_path, 'adatas')
		...
		for i, fold in enumerate(folds):
			...
			try:
				...
				if os.path.isfile(csv_path):
					...
					leiden_col = [col for col in train_df.columns if 'leiden' in col]
					train_df[leiden_col] = [hpc_label + s for s in list(train_df[leiden_col].values.astype(str))]
				...
				if h5_additional_path is not None:
					...
					csv_path = os.path.join(adatas_path_additional,   '%s.csv' % adata_name)
					if os.path.isfile(csv_path):
						additional_df = pd.read_csv(csv_path)
						leiden_col = [col for col in additional_df.columns if 'leiden' in col]
						additional_df[leiden_col] = [additional_hpc_label + s for s in list(additional_df[leiden_col].values.astype(str))]
						additional_df_frame_clusters, additional_df_frame_samples = create_frames(additional_df, groupby, meta_field, diversity_key=matching_field, reduction=2)
					frame_clusters = pd.concat([frame_clusters,additional_df_frame_clusters], axis = 0, ignore_index=True)
...
def run_logistic_regression(alphas, resolutions, meta_folder, meta_field, matching_field, folds_pickle, h5_complete_path, h5_additional_path, hpc_label, additional_hpc_label, force_fold, additional_as_fold, diversity_key=None,
	...
	adatas_path       = os.path.join(main_cluster_path, 'adatas')
	additional_cluster_path = h5_additional_path.split('hdf5_')[0]
	additional_cluster_path = os.path.join(additional_cluster_path, meta_folder)
	adatas_path_additional = os.path.join(additional_cluster_path, 'adatas')
	...
	for resolution in resolutions:
		...
		for i, fold in enumerate(folds):
			# Read CSV files for train, validation, test, and additional sets.
			dataframes, complete_df, leiden_clusters = read_csvs(adatas_path, adatas_path_additional, hpc_label, additional_hpc_label, matching_field, groupby, i, fold, h5_complete_path, h5_additional_path, additional_as_fold=additional_as_fold, force_fold=force_fold)
			train_df, valid_df, test_df, additional = dataframes
			...
			data, data_df, features = prepare_data_classes(dataframes, matching_field, meta_field, groupby, leiden_clusters, hpc_label, additional_hpc_label, type_composition, min_tiles,
	...
	for alpha in alphas:
		...
		for resolution in resolutions:
			...
			for i, fold in enumerate(folds):
				...
				# Convert meta field to integer
				frame_clusters[meta_field] = frame_clusters[meta_field].astype('float')
