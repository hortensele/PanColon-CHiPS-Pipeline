...
def bt_predictor(z_rep, z_dim, h_dim, num_class, spectral, activation, is_train, reuse, init='xavier', regularizer=None, normalization=None, name='encoder_predictor'):
	net = z_rep
	if display:
		print('PREDICTOR ENCODER INFORMATION:')
		print('Normalization: ', normalization)
	...
	with tf.variable_scope(name, reuse=reuse):
		...
		net = dense(inputs=net, out_dim=h_dim, spectral=spectral, init=init, regularizer=regularizer, scope=3)				
		if normalization is not None: net = normalization(inputs=net, training=is_train)
		net = activation(net)

		net = dense(inputs=net, out_dim=z_dim, spectral=spectral, init=init, regularizer=regularizer, scope=2)		
		if normalization is not None: net = normalization(inputs=net, training=is_train)
		net = activation(net)
        
		# Q Prediction.
		q_pred = dense(inputs=net, out_dim=num_class, spectral=spectral, init=init, regularizer=regularizer, scope='q_pred')		
