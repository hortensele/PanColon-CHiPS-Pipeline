...
def get_last_saved_epoch(data_out_path):
    generated_pt = sorted(glob.glob(os.path.join(data_out_path, 'results/epoch_*')), key=os.path.getctime, reverse=True)
    oldest_epoch_pt = generated_pt[0].split('_')[-1]
    return int(oldest_epoch_pt)
