...
class DeepZoomStaticTiler(object):
    ...
    def __init__(self, slidepath, basename, format, tile_size, overlap,
                limit_bounds, quality, workers, with_viewer, Bkg, basenameJPG, xmlfile, mask_type, ROIpc, oLabel, ImgExtension, SaveMasks, Mag, normalize, Fieldxml, pixelsize, pixelsizerange, Adj_WindowSize, resize_ratio, Best_level, Adj_overlap , Std, roi_coords_path):
        if with_viewer:
        ...
        self._Std = Std
        if pixelsize == 0.504:
            self._ROIcoords = pd.read_excel(roi_coords_path, sheet_name = "20x", engine='openpyxl',) # Added for excel sheet with roi coordinates
        elif pixelsize == 1.008:
            self._ROIcoords = pd.read_excel(roi_coords_path, sheet_name = "10x", engine='openpyxl',) # Added for excel sheet with roi coordinates
        elif pixelsize == 2.016:
            self._ROIcoords = pd.read_excel(roi_coords_path, sheet_name = "5x", engine='openpyxl',) # Added for excel sheet with roi coordinates
...
if __name__ == '__main__':
	...
	parser.add_option('-c', '--roicoordspath', metavar='NAME', dest='roi_coords_path',
