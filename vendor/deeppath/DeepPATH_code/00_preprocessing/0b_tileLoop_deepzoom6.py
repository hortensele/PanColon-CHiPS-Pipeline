...
def extract_slide_metadata(slide):
    """
    Robustly extract objective magnification and pixel size (um/px)
    from OpenSlide properties, including OME-TIFF/BTF metadata.
    Returns:
    ...
    # 1) Standard OpenSlide fields
    try:
        objective = float(props[openslide.PROPERTY_NAME_OBJECTIVE_POWER])
    except Exception:
        pass
    ...
    # 2) Parse OME XML from TIFF description/comment
    ome_xml = props.get("tiff.ImageDescription", None)
    if ome_xml is None:
        ome_xml = props.get("openslide.comment", None)
    ...
    if ome_xml:
        try:
            ...
            if objective is None:
                obj_node = root.find(".//ome:Objective", ns)
                if obj_node is not None and "NominalMagnification" in obj_node.attrib:
                    objective = float(obj_node.attrib["NominalMagnification"])

            if mpp_x is None or mpp_y is None:
                pix_node = root.find(".//ome:Pixels", ns)
                if pix_node is not None:
                    if mpp_x is None and "PhysicalSizeX" in pix_node.attrib:
                        mpp_x = float(pix_node.attrib["PhysicalSizeX"])
                    if mpp_y is None and "PhysicalSizeY" in pix_node.attrib:
                        mpp_y = float(pix_node.attrib["PhysicalSizeY"])
        except Exception as e:
            print("Warning: failed to parse OME XML metadata:", e)

    # 3) Legacy free-text fields
    img_desc = props.get("tiff.ImageDescription", "")
    if isinstance(img_desc, str) and "|" in img_desc:
        try:
            for nfields in img_desc.split("|"):
                if objective is None and "AppMag" in nfields:
                    objective = float(nfields.split(" = ")[1])
                if mpp_x is None and "MPP" in nfields:
                    mpp_x = float(nfields.split(" = ")[1])
                    mpp_y = mpp_x
        except Exception:
            pass

    return objective, mpp_x, mpp_y
...
class DeepZoomImageTiler(object):
    ...
    def _write_tiles(self):
        ...
        Objective, OrgPixelSizeX_meta, OrgPixelSizeY_meta = extract_slide_metadata(self._slide)

        if Objective is not None:
            print(self._basename + " - Obj information found: %.3f" % Objective)
        else:
            print(self._basename + " - No Obj information found")
            print(self._ImgExtension)
            if ("jpg" in self._ImgExtension) or ("dcm" in self._ImgExtension):
                Objective = 1.0
                Magnification = Objective
                print("input is jpg/dcm - will be tiled as such with %f" % Objective)
            else:
                return
        ...
        if True:
            ...
            if self._Mag <= 0:
                if self._pixelsize > 0:
                    ...
                    Objective_meta, OrgPixelSizeX, OrgPixelSizeY = extract_slide_metadata(self._slide)
                    if OrgPixelSizeX is None or OrgPixelSizeY is None:
...
if __name__ == '__main__':
	...
	for imgNb in range(len(files)):
		...
		if (opts.Mag <= 0) and (opts.pixelsizerange < 0):
			slide = open_slide(filename)
			# calculate the best window size before rescaling to reach desired final pizelsize
			Objective, OrgPixelSizeX, OrgPixelSizeY = extract_slide_metadata(slide)
			if OrgPixelSizeX is None or OrgPixelSizeY is None:
				print("Error: No pixel size information found in the header")
				continue
			if Objective is None:
				print("Warning: objective magnification not found; continuing with MPP only")
...
# 			print("info: Objective:" + str(Objective) + "; OrgPixelSizeX" + str(OrgPixelSizeX) + "; Desired_FoV_um: " + str(Desired_FoV_um) +"; Best_level: "+ str(Best_level) + "; resize_ratio: " +str(resize_ratio) + "; Adj_WindowSize:" + str(Adj_WindowSize) + "; self._tile_size: " + str(opts.tile_size),"; opts.overlap:", str(opts.overlap),"; Adj_overlap:", str(Adj_overlap))
		...
		if ("dcm" in ImgExtension) :
			...
			if opts.srh == 1 :
				opts.basenameJPG = os.path.basename(filename).split('.')[0]
