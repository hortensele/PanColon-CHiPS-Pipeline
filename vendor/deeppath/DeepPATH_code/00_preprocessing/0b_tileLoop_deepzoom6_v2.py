...
if __name__ == '__main__':
	...
	parser.add_option('-b', '--batch', type="string", dest='batch',
		help='if needed, subset TCGA dataset (100) prior to ')
	...
	subset_info = pd.read_csv(opts.batch, engine='python')
	files = [i for i in files if i.split("/")[-1] in subset_info.filename.values]
	if len(files)>0:    
		files = sorted(files)
